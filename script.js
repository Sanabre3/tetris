// === Configurações de grade e tamanho ===
// Tamanho (pixel) de cada célula da grade usada para posicionamento
const gridSpace = 30;

// === Estado global do jogo ===
// Peça que está caindo no momento
let fallingPiece;

// Peças já fixadas na grade
let gridPieces = [];

// Efeitos de fade para linhas removidas (caso existam)
let lineFades = [];

// Trabalhadores para animações/efeitos na grade
let gridWorkers = [];
...
// Ticks para controlar atualização/queda
let ticks = 0;
let updateEvery = 15;
...
// === Limites da área de jogo (coordenadas x) ===
const gameEdgeLeft = 150;
const gameEdgeRight = 450;

// === Paleta de cores das peças ===
// Cada índice corresponde a um tipo de peça (0..6)
const colors = [
  "#dca3ff",
  "#ff90a0",
  "#80ffb4",
...
// === Inicialização do p5.js ===
// Função `setup` é executada uma vez no começo e prepara o canvas e peças
function setup() {
...
// === Loop principal (desenhando e atualizando) ===
// Função `draw` é chamada repetidamente pelo p5.js; aqui ocorre o fluxo do jogo
function draw() {
...
// === Entrada do usuário ===
// `keyPressed` trata teclas como reiniciar (R) e mover/rotacionar a peça
function keyPressed() {
  if (keyCode === 82) {
    // 'R' key
    resetGame();
...
// === Classe PlayPiece ===
// Representa a peça que está caindo: posição, rotação, geração da próxima peça
class PlayPiece {
  constructor() {
...
// === Classe Square ===
// Cada pedaço da peça (quadrado) usado para desenhar e detectar colisões
class Square {
...
// === Gerador pseudo-randômico de próximas peças ===
// Garante variação entre peças; evita repetição imediata do mesmo tipo
...
// === Análise da grade e remoção de linhas completas ===
// Percorre a grade, remove linhas completas e atualiza pontuação/nível
...
// Retorna true se alguma linha foi removida (para repetir a verificação)
function checkLines() {
...
// === Classe Worker ===
// Responsável por animações/ajustes quando linhas são removidas (ex.: deslocamento)
class Worker {
...
// === Reinício do jogo ===
// Restaura todas as variáveis de estado para começar uma nova partida
function resetGame() {
  fallingPiece = new PlayPiece();
  fallingPiece.resetPiece();
  gridPieces = [];
